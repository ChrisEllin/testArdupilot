...
static mavlink_hil_state_quaternion_t last_hil_state_quaternion;
#endif
...
#elif HIL_SUPPORT
    if (plane.g.hil_mode == 1) {
		Quaternion Q(last_hil_state_quaternion.attitude_quaternion);
        mavlink_msg_simstate_send(chan,
                                  Q.get_euler_roll(),
                                  Q.get_euler_pitch(),
                                  Q.get_euler_yaw(),
                                  last_hil_state_quaternion.xacc*0.001f*GRAVITY_MSS,
                                  last_hil_state_quaternion.yacc*0.001f*GRAVITY_MSS,
                                  last_hil_state_quaternion.zacc*0.001f*GRAVITY_MSS,
                                  last_hil_state_quaternion.rollspeed,
                                  last_hil_state_quaternion.pitchspeed,
                                  last_hil_state_quaternion.yawspeed,
                                  last_hil_state_quaternion.lat,
                                  last_hil_state_quaternion.lon);
    }
#endif
}
...
void GCS_MAVLINK_Plane::handleMessage(const mavlink_message_t &msg)
{
    ...
    case MAVLINK_MSG_ID_HIL_STATE_QUATERNION:
    {
        ...
        last_hil_state_quaternion = packet;
       ...
       // plane.airspeed.setHIL(sq(vel.length()) / 2.0f + 2013);
		...
		AP_Baro::SimpleAtmosphere( packet.alt * .001f, sigma, delta, theta );
        plane.barometer.setHIL( 0, SSL_AIR_PRESSURE * delta, 303.16f * theta - C_TO_KELVIN, packet.alt * .001f, -packet.vz * .01f, AP_HAL::millis() );
		
		//Quaternion Q( packet.attitude_quaternion );
		float	roll = packet.attitude_quaternion[3],
				pitch = packet.attitude_quaternion[2],
				yaw = packet.attitude_quaternion[1];
        plane.compass.setHIL(0, roll, pitch, yaw);
        plane.compass.setHIL(1, roll, pitch, yaw);

        // cope with DCM getting badly off due to HIL lag
        if (plane.g.hil_err_limit > 0 &&
            (fabsf(roll - plane.ahrs.roll) > ToRad(plane.g.hil_err_limit) ||
             fabsf(pitch - plane.ahrs.pitch) > ToRad(plane.g.hil_err_limit) ||
             wrap_PI(fabsf(yaw - plane.ahrs.yaw)) > ToRad(plane.g.hil_err_limit))) {
            plane.ahrs.reset_attitude(roll, pitch, yaw);
        }
#endif
        break;
    }
